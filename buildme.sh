...
PKGNAME=webrockit-checksync
# this prefix will be added to all files in the package
INSTALLDIR=/opt/webrockit-checksync
...
fpm -s dir -t ${PKGTYPE} -n ${PKGNAME} -v ${VERSION} -d 'java-1.7.0-openjdk' --config-files './opt/webrockit-checksync/config/config.yml' --iteration ${ITERATION} -a ${PKGARCH} ./*
